...
Timeout supervision terminates the launched process when its deadline expires;
descendant processes are not guaranteed to be terminated as one race-free
process group, so callers must treat descendant cleanup as a residual limitation.
